    ...
    <div className="min-h-screen bg-[#f7f9fc] text-slate-800">
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 border-r border-slate-200/80 bg-white lg:flex lg:flex-col">
        ...
        <nav className="flex-1 overflow-y-auto px-3">
          ...
          <div className="mb-6">
            ...
            <Link href="/credits" className={`sidebar-link ${isActive(pathname, '/credits') ? 'sidebar-link-active' : ''}`}><Trophy size={18}/><span>Achievements & XP</span></Link>
          </div>
        </nav>
        <div className="border-t border-slate-100 p-4">
          <div className="flex items-center gap-3">
            ...
            <button onClick={signOut} title="Sign out" className="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-500"><LogOut size={16}/></button>
          </div>
        </div>
      </aside>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden">
        <nav className="mx-auto flex max-w-lg items-center justify-around">
          {[
            ['/dashboard','Home',Home], ['/learn','Learn',BookOpen], ['/matching','Matches',UsersRound],
            ['/messages','Messages',MessageCircle], ['/profile','Profile',UserRound]
          ].map(([href,label,Icon]) => <Link key={href} href={href} className={`mobile-tab ${isActive(pathname,href) ? 'mobile-tab-active' : ''}`}><Icon size={19}/><span>{label}</span></Link>)}
        </nav>
      </div>
