...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <section className="relative overflow-hidden rounded-[28px] bg-slate-950 px-6 py-8 text-white shadow-sm sm:px-8 lg:px-10 lg:py-10">
        ...
        <div className="relative max-w-3xl">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Good {getGreeting()}, {firstName} 👋</h1>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.5fr_1fr]">
        <div className="learning-goal-card">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="rounded-2xl bg-blue-50 p-3 text-blue-600"><Target size={21}/></div>
          </div>
          <div className="mt-6">
            <div className="mb-2 flex justify-between text-xs font-semibold"><span className="text-slate-500">Journey progress</span><span className="text-blue-700">{hasGoal ? '20%' : '0%'}</span></div>
            <div className="h-2.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full w-[20%] rounded-full bg-blue-600"/></div>
          </div>
          <div className="mt-5 flex items-center justify-between">
            ...
            <Link href={hasGoal ? '/matching' : '/profile'} className="inline-flex items-center gap-1 text-sm font-bold text-blue-600 hover:text-blue-700">
        ...
        <div className="sb-card p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="rounded-2xl bg-violet-50 p-3 text-violet-600"><GraduationCap size={21}/></div>
          </div>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="sb-card p-6">
          ...
          {nextSession ? <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
            <div className="flex items-start justify-between"><div><span className="badge-blue">Learning session</span><h3 className="mt-3 font-bold">{nextSession.title || 'Skill exchange session'}</h3></div><CalendarDays className="text-blue-600" size={20}/></div>
            ...
            <Link href="/sessions" className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-blue-700">Open session <ArrowRight size={14}/></Link>
          </div> : <div className="mt-4 rounded-2xl bg-slate-50 p-5 text-center"><CalendarDays className="mx-auto text-slate-300" size={26}/><p className="mt-2 text-sm font-semibold">No upcoming sessions</p><Link href="/matching" className="mt-2 inline-block text-xs font-bold text-blue-600">Find a learning partner</Link></div>}
        </div>
      </section>
...
function SectionHeading({title,subtitle,href,action}) {
  return <div className="mb-4 flex items-end justify-between gap-4"><div><h2 className="text-xl font-extrabold tracking-tight text-slate-900">{title}</h2><p className="mt-1 text-sm text-slate-500">{subtitle}</p></div>{href && <Link href={href} className="hidden shrink-0 items-center gap-1 text-sm font-bold text-blue-600 sm:flex">{action}<ArrowRight size={14}/></Link>}</div>;
}
...
function LearningCard({skill,index}) { const progress=[70,45,25][index%3]; return <div className="course-card"><div className={`course-cover cover-${index%3}`}><span>{['Learning path','Skill practice','Peer learning'][index%3]}</span><BookOpen size={30}/></div><div className="p-4"><div className="flex items-center justify-between gap-2"><h3 className="truncate font-bold text-slate-800">{skill.name}</h3><span className="text-[10px] font-bold text-slate-400">{skill.proficiency || 'Beginner'}</span></div><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{skill.description || 'Build practical knowledge through a peer learning exchange.'}</p><div className="mt-4"><div className="mb-1 flex justify-between text-[10px] font-semibold text-slate-400"><span>Progress</span><span>{progress}%</span></div><div className="h-1.5 rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600" style={{width:`${progress}%`}}/></div></div><Link href="/matching" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-blue-600">Continue <ArrowRight size={13}/></Link></div></div>; }
function MatchPreview({match}) { return <div className="flex items-center gap-3 rounded-2xl border border-slate-100 p-3 hover:border-blue-100 hover:bg-blue-50/30"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-700">{(match.name || match.candidateName || 'SB').split(' ').map(x=>x[0]).slice(0,2).join('')}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{match.name || match.candidateName || 'SkillBridge member'}</p><p className="truncate text-xs text-slate-500">{match.teachSkill || match.skillToTeach || 'Skill exchange partner'}</p></div><span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">{Math.round(match.score || match.compatibility || 0)}% match</span></div>; }
function EmptyLearning(){return <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center"><BookOpen className="mx-auto text-slate-300" size={28}/><p className="mt-2 font-bold">Your learning shelf is empty</p><p className="mt-1 text-sm text-slate-500">Add a skill you want to learn to start building your journey.</p><Link href="/profile" className="mt-4 inline-flex items-center gap-1 rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white">Add learning goals <ArrowRight size={14}/></Link></div>}
