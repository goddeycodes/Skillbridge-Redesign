...
export default function LearnPage(){
 ...
 return <div className="space-y-8">
  ...
  <section><div className="flex flex-wrap gap-2">{cats.map(c=><button key={c} onClick={()=>setCategory(c)} className={`rounded-full px-4 py-2 text-xs font-bold transition ${category===c?'bg-brand-600 text-white':'border border-slate-200 bg-white text-slate-500 hover:border-brand-200 hover:text-brand-600'}`}>{c}</button>)}</div></section>
  <section>
   <div className="mb-4 flex items-end justify-between"><div><h2 className="text-xl font-extrabold">Skills you can learn</h2><p className="mt-1 text-sm text-slate-500">Learn with a person, not just a playlist.</p></div><Link href="/profile" className="hidden items-center gap-1 text-sm font-bold text-brand-600 sm:flex">Set learning goals <ArrowRight size={14}/></Link></div>
